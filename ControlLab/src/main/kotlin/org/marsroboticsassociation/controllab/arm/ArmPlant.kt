 ...
 *
 * The rigid plant reports `motorPos == loadPos` and `isEngaged == true` so the arm
 * renderer can always draw both a load link and a motor-side link uniformly; only the lashy plants
 * make them diverge. The flex plant reports the *tip* as the true load — the thing that
 * visibly bounces.
 */
interface ArmPlant {
    /**
     ...
     * always seed at rest, so `loadVel` is advisory and the new plant starts with zero
     * velocity.
     */
