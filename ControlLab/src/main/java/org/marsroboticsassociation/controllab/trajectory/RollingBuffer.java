...
/**
 * Fixed-capacity circular buffer that evicts entries older than {@code windowSeconds} from the head
...
public class RollingBuffer {
    ...
    public void add(
            double time, double position, double velocity, double acceleration, double target) {
    ...
    public List<Double> getVelocities() {
        return toList(velocities);
    }

    public List<Double> getAccelerations() {
        return toList(accelerations);
    }

    public List<Double> getTargets() {
        return toList(targets);
    }
