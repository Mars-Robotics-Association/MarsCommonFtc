...
/**
 * The simulated sysid must recover the plant's true feedforward coefficients (especially `kA`, the
 * inertia the model-based controller needs) from encoder logs alone.
 */
...
class ArmSysIdTest {
    ...
    fun recoversDefaultHeavyArmParams() {
        val cfg = ArmPlantConfig() // kS=0.3, kG=3.5, kV=1.2, kA=0.35
        val r = ArmSysId.characterize(cfg)
        System.out.printf(
    ...
    @Test
    fun engineAppliesIdentifiedInertiaToMechanismModel() {
        ...
        val e = ArmEngine(ArmControllerType.MECHANISM_PIDF, 7L)
        ...
        assertEquals(
            ...
            e.getMechGains().kA,
            0.7 * 0.15,
            "mechanism kA should match the new plant",
        )
        assertEquals(
            2.8,
            e.getMechGains().kCos,
            2.8 * 0.10,
            "mechanism gravity should match the plant",
        )
    }

    @Test
    fun recoversAModifiedInertia() {
        val cfg = ArmPlantConfig()
    ...
    @Test
    fun recoversThroughBacklashFromTheMotorEncoder() {
        ...
        val cfg = ArmPlantConfig() // kA=0.35, kG=3.5
        cfg.backlashRad = Math.toRadians(12.0)
        val r = ArmSysId.characterize(cfg, /* throughBacklash= */ true)
        System.out.printf(
            "sysid(backlash 12°): kS=%.3f kV=%.3f kA=%.3f kCos=%.3f R2=%.4f%n",
            r.kS,
            r.kV,
            r.kA,
            r.kCos,
        ...
        assertTrue(
            r.rSquared > 0.99,
            "fit through backlash should still be excellent, R2=" + r.rSquared,
        )
    ...
    @Test
    fun recoversThroughFlexFromTheMotorEncoder() {
        ...
        val cfg = ArmPlantConfig() // kA=0.35, kG=3.5, flex 3 Hz zeta 0.03
        val r = ArmSysId.characterize(cfg, ArmEngine.PlantKind.FLEX)
        System.out.printf(
        ...
        assertTrue(!r.kVHold.isNaN(), "hold-side kV should be pinned by the speed sweep")
    }

    @Test
    fun engineSysIdUsesTheBacklashPlantWhenEnabled() {
