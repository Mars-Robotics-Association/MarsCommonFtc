 ...
 * [MechanismModel] at its *own* current state and rewrites its velocity, acceleration, and
 * deceleration limits itself. [MotorMechanismController] drives this [ModelAwareSetpointProfile]
 * surface and only forwards the available voltage each loop.
 *
 * <p>Owning the ceilings lets each plan use limits evaluated at the exact state it plans from (see
 * [ModelAwareSetpointProfile] for why an outside caller cannot). Two ceilings are handled
 * specially:
 * <ul>
 * <li><b>Velocity, with one-step lookahead.</b> The sustainable-velocity ceiling depends on
 *   position through gravity, so a profile cruising exactly at the ceiling can find itself a hair
 *   *above* the ceiling one step later wherever the ceiling falls along the motion — a regime that
 *   forces the clamp-and-replan fallback every loop. The ceiling used for each plan is the minimum
 *   of its value here and at the position one step ahead along the current motion, so the plan
 *   never cruises into a falling ceiling.
 * <li><b>Braking, at zero speed and at the target.</b> Back-EMF aids braking, so the model's
 *   deceleration ceiling shrinks as a stop progresses — and it also varies with position through
 *   gravity, collapsing toward the target on a gravity-loaded descent. A planner assumes its limits
 *   hold for the whole remaining move, so the stop is planned at the ceiling's minimum over the
 *   stop: evaluated at rest, at the worse of the current and the target position. </li>
 * </ul>
 *
 * <p>The configured limits passed at construction remain the mechanical caps; the model ceilings
 * only ever lower them. [setAvailableVoltage] must be called before each [update] (the controller
 * does); until it is called there is no voltage, hence no authority, and the setpoint holds.
 */
class ModelAwareRuckigProfiler : RuckigProfiler, ModelAwareSetpointProfile {
     ...
     * braking (motion-frame, per the [SetpointProfile] contract). Pass `maxAcceleration >
     * maxDeceleration` to let a mechanism launch harder than it stops; the back-EMF ceilings still
     * throttle each cap independently below its configured value.
     *
    ...
    override fun update(targetPosition: Double, dt: Double) {
        ...
        val travelDirection = targetPosition - currentPosition
        ...
        val motionDirection = if (currentVelocity != 0.0) currentVelocity else travelDirection
        ...
        val decelCeiling =
            min(
                model.maxSustainableDeceleration(availableVoltage, currentPosition, 0.0),
        ...
        val ceilingHere =
            model.maxSustainableVelocity(availableVoltage, currentPosition, travelDirection)
        val ceilingAhead =
