...
class RuckigProfilerTest {
    ...
    fun stopIsJerkBoundedAllTheWayIntoTheTarget() {
        ...
        var prevV = p.getVelocity()
        var maxJerkObserved = 0.0
        var steps = 0
        while (steps < 2000) {
            p.update(1.0, DT)
            steps++
            maxJerkObserved = max(maxJerkObserved, abs(p.getAcceleration() - prevA) / DT)
            assertTrue(abs(p.getVelocity()) <= maxV + 1e-9, "velocity limit")
            assertTrue(
                abs(p.getVelocity() - prevV) <= max(maxA, maxD) * DT + 1e-9,
    ...
    fun survivesBackEmfStyleAccelDecay() {
        ...
        var prevP = p.getPosition()
        var prevA = p.getAcceleration()
        var settled = false
        for (i in 0 until 1500) {
            val speed = abs(p.getVelocity())
            p.setMaxAcceleration(max(0.0, 4.0 - 1.8 * speed))
            p.setMaxDeceleration(2.0) // conservative braking ceiling
            p.update(3.0, DT)
            assertTrue(
                abs(p.getPosition() - prevP) <= maxV * DT + 1e-9,
                "position continuous at step $i",
            )
            assertTrue(
                abs(p.getAcceleration() - prevA) <= maxJ * DT * 1.05 + 1e-9,
    ...
    fun velocityCapDroppedBelowCurrentSpeedBrakesViaPreTrajectory() {
        ...
        for (i in 0 until 1500) {
            p.update(10.0, DT)
            assertTrue(
                abs(p.getVelocity() - prevV) <= 4.0 * DT + 1e-9,
    ...
    fun velocityCeilingChatterDoesNotFreezeTheProfile() {
        ...
        val posBefore = p.getPosition()
        val velBefore = p.getVelocity()
        for (i in 0 until 60) {
        ...
        val advanced = p.getPosition() - posBefore
        assertTrue(
    ...
    fun zeroVelocityAuthorityHoldsPosition() {
        ...
        assertTrue(abs(p.getPosition()) < 1e-6, "no authority: setpoint holds")
    }
    ...
    fun unlimitedJerkRunsSecondOrderProfiles() {
        ...
        while (steps < 1000) {
            ...
            steps++
            assertTrue(
                abs(p.getVelocity() - prevV) <= maxA * DT * 1.05 + 1e-9,
