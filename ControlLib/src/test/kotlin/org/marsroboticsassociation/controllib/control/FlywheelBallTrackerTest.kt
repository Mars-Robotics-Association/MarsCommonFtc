 ...
 *
 * <p>Measurements are injected directly — no motor hardware or plant simulation is required. All
 * scenarios use a fixed dt = 10 ms (100 Hz), matching a typical robot loop period. 10 TPS Gaussian
 * noise is added to every measurement to exercise the filter under realistic encoder jitter
 * (matching the noise level used by the other flywheel test suites).
 *
 * <p>Physics recap: the disturbance state `d` represents ball drag in TPS-equivalent units. A
 * sustained measurement drop below the setpoint drives `d` positive. When `d` exceeds
 * `detectionThreshold` (default 50 TPS) the ball is considered engaged; it clears when `d` falls
 * below `exitThreshold` (default 20 TPS).
 */
class FlywheelBallTrackerTest {

    private lateinit var motorConfig: VelocityMotorPF.VelocityMotorPFConfig
    private lateinit var tracker: FlywheelBallTracker
    private lateinit var rng: Random

    @BeforeEach
    fun setUp() {
        // Isolate tests: fresh default config and a new tracker each time.
        FlywheelBallTracker.config = FlywheelBallTracker.Config()
        motorConfig = VelocityMotorPF.VelocityMotorPFConfig()
        tracker = FlywheelBallTracker(motorConfig)
        rng = Random(SEED)
    }

    /** Feed a constant measurement + noise for [steps] iterations at [SETPOINT_TPS]. */
    private fun run(measurementTps: Double, steps: Int) {
        for (i in 0 until steps) {
            val noisy = measurementTps + rng.nextGaussian() * NOISE_STD_TPS
    ...
    @Test
    fun freshFilter_allStateIsZero() {
    ...
    @Test
    fun measurementDrop_raisesDAndDetectsBall() {
        ...
        System.out.printf(
            "measurementDrop_raisesDAndDetectsBall: d=%.1f TPS%n",
        ...
        assertTrue(
            tracker.getDisturbance() > FlywheelBallTracker.config.detectionThreshold,
    ...
    @Test
    fun hysteresis_detectionPersistsThenClears() {
        ...
        System.out.printf(
            "hysteresis_detectionPersistsThenClears: d=%.1f TPS after recovery%n",
            tracker.getDisturbance(),
        )
        assertFalse(
            tracker.isBallDetected(),
        ...
        assertTrue(
            tracker.getDisturbance() < FlywheelBallTracker.config.exitThreshold,
    ...
    companion object {
        private const val DT = 0.010 // 10 ms loop period
        private const val SETPOINT_TPS = 2000.0 // typical flywheel setpoint
        private const val NOISE_STD_TPS = 10.0
